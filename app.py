...
def load_data():
    ...
    urls = [
        ...
        "https://www.football-data.co.uk/mmz4281/2324/E0.csv"  # PL Anterior
    ]
    frames = []
    for u in urls:
        try:
...
tactical_dna = {
    ...
    "Everton": {"style": "Physical", "w": 4, "panic": 8, "agg": 6, "aerial": 10},
...
def get_advanced_stats(team, df, n=6):
    ...
    return {
        ...
        "momentum": np.mean(goals)
    }

def get_weather(team):
    coords = stadiums.get(team, (51.5, -0.1))
    try:
...
weather_boost = 0
if rain > 1.0: weather_boost += 0.8
if wind > 25.0: weather_boost -= 1.2

state_boost = 0
if sim_minute > 15:
    if sim_score_diff == "Empate": state_boost += 0
    elif sim_score_diff == "Visita Gana por 1" and dna_h.get('agg', 5) > 7: state_boost += 2.5
    elif sim_score_diff == "Local Gana por 2+": state_boost -= 1.5

final_corners = base_corners + tactical_boost + weather_boost + state_boost

# CÁLCULO DE TARJETAS
base_cards = (stats_h['avg_card'] + stats_a['avg_card']) / 2 + 1.5
friction = (stats_h['avg_foul'] + stats_a['avg_foul']) / 22 
